...
const CreateRecipeModal = ({ projectId, project, onClose, onCreated }) => {
  ...
  const selectCombination = (index, comboId) => {
    ...
    const alreadyUsed = combinations.some(
      (c, i) => c.id === comboId && i !== index,
    );

    if (alreadyUsed) {
      setFormError("Combination already added");
      return;
    }
  ...
  const submit = async () => {
    ...
    if (combinations.length === 0) {
      setFormError("Add at least one combination");
      return;
    }

    if (combinations.some((c) => !c.id)) {
      setFormError("All combinations must be selected");
      return;
    }
  ...
  const createRecipe = async () => {
    try {
      ...
      await api.post(`/api/recipes/projects/${projectId}/recipes/create/`, {
        name,
        combinations,
      });
        ...
        <div className="modal-card large">
          ...
          <div className="form-group">
            ...
            <button
              ...
              disabled={
                loading || loadingCombinations || allCombinations.length === 0
              }
            >
              + Add Combination
            </button>
                        ...
                        >
                          ...
                          <tbody>
                            {combo.tag_values.map((tv, tagIndex) => (
                              <tr key={tv.tag_id}>
                                <td>
                                  {tags.find((t) => t.id === tv.tag_id)?.name ||
                                    `Tag ${tv.tag_id}`}
                                </td>
                                <td>
                                  <input
